...
int64 nChainStartTime = 1402021721;
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    nRewardCoinYear = max(nRewardCoinYear,(int64)(MAX_MINT_PROOF_OF_STAKE * 0.01));
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in E-Gold
    bool fStrictPayToScriptHash = true; // Always active in E-Gold
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            printf("ProcessBlock: ORPHAN BLOCK, missing blocks, from %d to %s\n", pindexBest->nHeight,hash.ToString().substr(0,20).c_str());
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            else if (inv.type == MSG_BLOCK && mapOrphanBlocks.count(inv.hash)) 
            {
                ...
                printf("Message inv PushGetBlocks: %d to %s\n", pindexBest->nHeight,inv.hash.ToString().c_str());
            } 
        ...
        if (nSearchTime > nLastCoinStakeSearchTime)
        {
            ...
            if (pwallet->CreateCoinStake(*pwallet, pblock->nBits, nSearchTime-nLastCoinStakeSearchTime, txCoinStake))
            {
                ...
                {   // make sure coinstake would meet timestamp protocol
                 ...
                 //printf("!pwallet->CreateCoinStake\n");
            }
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            ...
            if (pblock->IsProofOfStake())
            {
                ...
                {
                    cout<<"!pblock->SignBlock(*pwalletMain)"<<endl;
                    printf("!pblock->SignBlock(*pwalletMain)=%s\n", strMintMessage.c_str());
                    strMintWarning = strMintMessage;
                    continue;
                }
                strMintWarning = "";
                cout<<"CPUMiner : proof-of-stake block found"<<pblock->GetHash().ToString().c_str()<<endl;
                printf("CPUMiner : proof-of-stake block found %s\n", pblock->GetHash().ToString().c_str());
                SetThreadPriority(THREAD_PRIORITY_NORMAL);
                CheckWork(pblock.get(), *pwalletMain, reservekey);
                SetThreadPriority(THREAD_PRIORITY_LOWEST);
            }else{
                //printf("!pblock->IsProofOfStake()\n");
                cout<<!pblock->IsProofOfStake()<<endl;
            }
            Sleep(500);
            continue;
        }
